...
for param in range(14):
    ...
    for kt in kats:
        ...
        for train, test in skf.split(X,labels):
            ...
            l1Test = np.linalg.norm(np.subtract(X[test], centerTrain), ord=1, axis=1, keepdims=True)
            tempTest = np.append(X[test], l1Test, axis=1)
            timeF.append(time.time())
            acc.append(accuracy_score(labels[test], svc.predict(tempTest)))
            accTrain.append(accuracy_score(labels[train], svc.predict(tempX)))
